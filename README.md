...
- 단일 화면내에서 3개의 section으로 구성
  - 차트
    ...
    - 약 6시간 소요 => tanstack query로 개별 데이터를 받을 수 있는게 아니라 전체 데이터를 받아 오기에 zustand를 통해 현 차트에 표시된 id를 저장하고 전역에서 공유하여 서로 다른 section에서 같은 id의 값을 보이도록 설계
  - 테이블
    - 차트에 나타나는 지표를 세분화하여 테이블로 시각화
    - 약 4시간 소요 => 서로 다른 유형의 데이터(전기,원소재,운송 등)의 단위를 잘 매치되도록 설계
  - 포스트 폼
    - 선택된 회사의 지표에 대해 자유롭게 피드백 할 수 있는 폼 제작
    - 약 4시간 소요 => create/update 로직을 한 컴포넌트로 작성하여 재사용 가능하도록 설계
